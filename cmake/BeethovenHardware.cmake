...
function(beethoven_hardware TARGET)
  cmake_parse_arguments(BH
    ""                              # Boolean options
    "MAIN_CLASS;PLATFORM;BUILD_MODE" # Single-value args
  ...
  # Set defaults
  if(NOT BH_PLATFORM)
    set(BH_PLATFORM "discrete")
  endif()
  if(NOT BH_BUILD_MODE)
    set(BH_BUILD_MODE "Simulation")
  endif()

  # Create local BEETHOVEN_PATH for this target
  set(LOCAL_BEETHOVEN_PATH "${CMAKE_SOURCE_DIR}")
  set(LOCAL_BUILD_DIR "${LOCAL_BEETHOVEN_PATH}/build")
  set(LOCAL_HW_DIR "${LOCAL_BUILD_DIR}/gen")

  # Generated output files
  set(GENERATED_HEADER "${LOCAL_BUILD_DIR}/beethoven_hardware.h")
  set(GENERATED_SOURCE "${LOCAL_BUILD_DIR}/beethoven_hardware.cc")
  set(GENERATED_CMAKE_SRCS "${LOCAL_BUILD_DIR}/cmake_srcs.cmake")
  set(GENERATED_VCS_SRCS "${LOCAL_BUILD_DIR}/vcs_srcs.in")

  # Create output directories
  file(MAKE_DIRECTORY "${LOCAL_BUILD_DIR}")
  ...
  # Create stamp file to track successful generation
  set(STAMP_FILE "${LOCAL_BUILD_DIR}/.beethoven_generated_${TARGET}")


  # Export variables for use by beethoven_testbench()
  set(${TARGET}_BEETHOVEN_PATH "${LOCAL_HW_DIR}/${TARGET}_HW_DIR/" PARENT_SCOPE)
  ...
  add_custom_command(
    ...
    COMMAND ${CMAKE_COMMAND} -E echo "  Platform: ${BH_PLATFORM}"
    COMMAND ${CMAKE_COMMAND} -E echo "  Build mode: ${BH_BUILD_MODE}"
    COMMAND ${CMAKE_COMMAND} -E echo "  Output: ${LOCAL_BEETHOVEN_PATH}"
    COMMAND ${CMAKE_COMMAND} -E env
    "BEETHOVEN_PATH=${LOCAL_HW_DIR}/${TARGET}_HW_DIR/"
    sbt "${SBT_RUN_CMD}"
  ...
  # Create a custom target for the hardware generation
  add_custom_target(${TARGET}_hw
    DEPENDS "${STAMP_FILE}"
  )
...
function(beethoven_testbench TARGET)
    ...
    if(NOT DEFINED ${BT_HARDWARE}_BEETHOVEN_PATH)
        message(FATAL_ERROR
            "beethoven_testbench: '${BT_HARDWARE}' is not a valid beethoven_hardware target.\n"
    ...
    set(HW_BUILD_DIR "${${BT_HARDWARE}_BUILD_DIR}")
    ...
    add_custom_target(
      ${TARGET}_sim
      COMMAND make -f ${_BEETHOVEN_PREFIX}/share/beethoven/runtime/Makefile SIMULATOR=${BT_SIMULATOR} RUNTIME_DIR=${_BEETHOVEN_PREFIX}/share/beethoven/runtime STATIC_LIB=0
      COMMAND mv BeethovenSim ${TARGET}_sim
      WORKING_DIRECTORY "${CMAKE_BUILD_DIR}"
      VERBATIM
    )

    add_dependencies(${TARGET}_sim
      ${BT_HARDWARE}_hw)

   beethoven_build(${TARGET}
      SOURCES ${BT_SOURCES}
      PATH ${${BT_HARDWARE}_HW_DIR})
