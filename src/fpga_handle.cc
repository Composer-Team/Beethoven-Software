...
fpga_handle_sim_t::fpga_handle_sim_t(int id) {
...
void fpga_handle_sim_t::check_rc(int rc, std::string infostr) {}

void fpga_handle_real_t::check_rc(int rc, std::string infostr) {
  if (rc) {
...
void fpga_handle_real_t::fpga_shutdown() {
...
void fpga_handle_sim_t::fpga_shutdown() {}

void fpga_handle_real_t::fpga_setup(int id) {
  /*
...
void fpga_handle_t::store_resp(uint32_t rd, uint32_t unit_id, uint32_t retval) {
  rocc_resp_table[rd][unit_id] = retval;
}

uint32_t fpga_handle_t::resp_lookup(uint32_t rd, uint32_t unit_id) {
  return rocc_resp_table[rd][unit_id];
}

int fpga_handle_real_t::get_write_fd() {
  return xdma_write_fd;
}

int fpga_handle_sim_t::get_write_fd() {
  return int(NULL);
}

int fpga_handle_real_t::get_read_fd() {
  return xdma_read_fd;
}

int fpga_handle_sim_t::get_read_fd() {
  return int(NULL);
}

void fpga_handle_real_t::write(size_t addr, uint32_t data) {
...
void fpga_handle_sim_t::write(size_t addr, uint32_t data) {
  // addr is really a (32-byte) word address because of zynq implementation
  addr <<= 2;
...
uint32_t fpga_handle_real_t::read(size_t addr) {
  addr <<= 2;
  uint32_t value;
...
uint32_t fpga_handle_sim_t::read(size_t addr) {
  addr <<= 2;
  // printf("addr: %#x\n", addr);
  uint64_t cmd = addr;
  char * buf = (char*)&cmd;
...
uint32_t fpga_handle_real_t::is_write_ready() {
  uint64_t addr = 0x4;
  uint32_t value;
  int rc = fpga_pci_peek(pci_bar_handle, addr, &value);
...
uint32_t fpga_handle_sim_t::is_write_ready() {
  uint64_t addr = 0x4;
  uint64_t cmd = addr;
  char * buf = (char*)&cmd;
  ::write(driver_to_xsim_fd, buf, 8);
...
bool fpga_handle_sim_t::is_real() {return false;}

bool fpga_handle_real_t::is_real() {return true;}
