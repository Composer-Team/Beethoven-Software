...
rocc_response fpga_handle_sim_t::get_response_from_handle(int handle) const {
...
response_handle fpga_handle_sim_t::send(const rocc_cmd &c) const {
...
remote_ptr fpga_handle_sim_t::malloc(size_t len) {
  // acquire lock over client side
...
void fpga_handle_sim_t::copy_to_fpga(const remote_ptr &dst, const void *host_addr) {
...
void fpga_handle_sim_t::copy_from_fpga(void *host_addr, const remote_ptr &src) {
...
void fpga_handle_sim_t::free(remote_ptr ptr) {
  pthread_mutex_lock(&data_server->data_cmd_send_lock);
