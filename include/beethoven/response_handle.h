...
public:
#ifndef BAREMETAL
  explicit response_getter(bool cw, int id, const fpga_handle_t &h)
      : id(id), can_wait(cw), h(&h) {};
#else
...
private:
  std::optional<rocc_response> get_help(const bool &block);
};

template <typename t> class response_handle {
...
public:
  ...
  template <typename s> response_handle<s> to() {
...
std::ostream &operator<<(std::ostream &os,
                         const beethoven::rocc_response &response);
#endif
