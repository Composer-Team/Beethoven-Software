//
// Created by Christopher Kjellqvist on 10/21/24.
//
...
class [[maybe_unused]] device_allocator {
  ...
  constexpr static uint8_t log_superblock_size = log2up(superblock_size);
  constexpr static uint8_t num_lists = log_superblock_size - log_min_block;
  constexpr static uint64_t log_num_superblocks = addr_bits - log_superblock_size;
  constexpr static uint64_t num_superblocks = 1 << log_num_superblocks;

  uint64_t superblock_id_mask() {
    uint64_t mask = 1;
  ...
  const int superblock_free_idx_lim = 1 << (addr_bits - log_superblock_size);
...
//  std::array<superblock, num_superblocks> superblocks;
  ...
  block_info get_block_info(uint64_t fpga_addr) {
...
#endif //BEETHOVEN_DEVICE_ALLOCATOR_H
